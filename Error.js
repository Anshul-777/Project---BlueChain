...
const MIN_IMAGE_KB = 10; // accept any size >=10KB and <=max
...
Thought for 55s
Fixed only the four issues you flagged, keeping everything else unchanged. Below are the updated files.
...
const MAX_DOC_MB = 20;
const MAX_BOUNDARY_MB = 20;
const MIN_IMAGE_KB = 10; // accept from 10KB up to max
...
const RegisterForm = () => {
  ...
  // Local form
  const [localData, setLocalData] = useState(getInitialLocal());
  const [localErrors, setLocalErrors] = useState({});

  // Organization form
  const [orgData, setOrgData] = useState(getInitialOrg());
  ...
  const validateLocal = () => {
    ...
    if (!Array.isArray(d.photos) || d.photos.length < 2) {
      errs.photos = "Upload at least 2 photos (JPG/PNG, 10 KB to 10 MB each).";
    } else {
  ...
  const validateOrg = () => {
    ...
    if (d.soilOrganicCarbonPercent && !within(d.soilOrganicCarbonPercent, 0, 60))
      errs.soilOrganicCarbonPercent = "SOC% should be 0–60.";
  ...
  const hardResetForm = () => {
    if (projectType === "Local") {
      setLocalData(getInitialLocal());
      setLocalErrors({});
    } else {
      setOrgData(getInitialOrg());
      setOrgErrors({});
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (projectType === "Local") {
        ...
        /* Optional SHA-256 (commented)
        const core = { type: "local", projectTitle: d.projectTitle, ownerName: d.ownerName,
          ownerPhone: d.ownerPhone, ownerEmail: d.ownerEmail, country: d.country,
          placeName: d.placeName, lat: d.lat, lng: d.lng, areaHa: d.areaHa,
          ecosystems: d.ecosystems, plantTypes: d.plantTypes, approxPlants: d.approxPlants, startDate: d.startDate };
        ...
        */
        ...
        /* Optional SHA-256 (commented)
        const core = { type: "org", projectTitle: d.projectTitle, organizationName: d.organizationName,
          orgContactName: d.orgContactName, orgContactEmail: d.orgContactEmail, startDate: d.startDate,
          placeName: d.placeName, lat: d.lat, lng: d.lng, areaHa: d.areaHa, ecosystems: d.ecosystems,
          speciesList: d.speciesList, methodology: d.methodology, monitoringPlan: d.monitoringPlan };
        // const bytes = new TextEncoder().encode(JSON.stringify(core));
        // const hashBuf = await window.crypto.subtle.digest("SHA-256", bytes);
        // const clientSha256 = Array.from(new Uint8Array(hashBuf)).map(b=>b.toString(16).padStart(2,"0")).join("");
        // payload.append("clientSha256", clientSha256);
      ...
      setTimeout(() => {
        ...
        window.location.href = "/home"; // return to HomePage.js
      }, 1600);
    } catch (err) {
  ...
  return (
    <div className={`container py-4 register-form-root ${theme === "dark" ? "theme-dark" : "theme-light"}`} data-theme={theme}>
      <style>{`
        :root { --primary-500:#0ea5e9; --indigo-500:#6366f1; --emerald-500:#10b981; --rose-500:#f43f5e; --slate-900:#0f172a;
                --slate-800:#1e293b; --slate-700:#334155; --slate-100:#f1f5f9; --card-bg:rgba(255,255,255,0.92);
                --text-color:#0b1220; --muted:#6b7280; --border:rgba(0,0,0,0.06); }
        [data-theme='dark'] { --card-bg:rgba(15,23,42,0.92); --text-color:#e5e7eb; --muted:#93a3b8; --border:rgba(255,255,255,0.08); }
        .bg-gradient-1 { background: linear-gradient(135deg, #4f46e5, #0ea5e9); }
        .card-neo { background: var(--card-bg); color: var(--text-color); backdrop-filter: blur(6px);
                   border:1px solid var(--border); border-radius:16px; box-shadow:0 10px 30px rgba(2,6,23,0.12);
        ...
        .section { padding:16px; border-radius:12px; border:1px dashed rgba(99,102,241,0.35);
                   background:linear-gradient(0deg, rgba(99,102,241,0.08), rgba(14,165,233,0.07)); margin-bottom:16px; }
        [data-theme='dark'] .section { background:linear-gradient(0deg, rgba(99,102,241,0.18), rgba(14,165,233,0.12)); }
        ...
        .success-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.6); display:flex; align-items:center; justify-content:center; z-index:9999; }
        .success-card { background:#fff; padding:24px 28px; border-radius:16px; text-align:center; animation: popIn .35s ease both; }
        @keyframes popIn { from { transform: scale(0.92); opacity:0;} to { transform: scale(1); opacity:1;} }
        .checkmark { width:64px; height:64px; border-radius:50%; background:var(--emerald-500); display:inline-flex; align-items:center; justify-content:center; color:#fff; font-size:38px; margin-bottom:12px; box-shadow:0 8px 20px rgba(16,185,129,0.4); animation:pulse .9s ease 1; }
        ...
        .hold-tip-bubble { position:absolute; top:-8px; left:20px; min-width:220px; max-width:320px; background:var(--primary-500); color:#fff; padding:8px 10px; border-radius:8px; font-size:12px; box-shadow:0 6px 14px rgba(14,165,233,0.35); animation: tipFade .2s ease both; z-index:10; }
        ...
        .upload-animated { border:2px dashed rgba(14,165,233,0.55); padding:10px; border-radius:10px; transition:border-color .25s, box-shadow .25s, background-color .25s; background:rgba(14,165,233,0.06); }
        .upload-animated:hover { border-color:var(--indigo-500); background:rgba(99,102,241,0.08); box-shadow:0 0 0 6px rgba(99,102,241,0.12); }
        .themed-btn { transition: transform .15s ease, box-shadow .2s ease; }
        .themed-btn:hover { transform: translateY(-1px); box-shadow: 0 8px 20px rgba(14,165,233,0.25); }
        .theme-toggle { background:rgba(255,255,255,0.25); border:1px solid rgba(255,255,255,0.35); color:#fff; }
        .theme-toggle:hover { background:rgba(255,255,255,0.35); }
        .signature-box { position:relative; border:2px dashed rgba(99,102,241,0.45); border-radius:10px; height:160px; background:rgba(99,102,241,0.05); overflow:hidden; }
        .signature-canvas { width:100%; height:100%; display:block; }
        .signature-watermark { position:absolute; top:8px; right:12px; font-size:12px; color:var(--muted); }
      `}</style>
          ...
          <Form noValidate onSubmit={handleSubmit}>
            ...
            {projectType === "Local" && (
              <>
                ...
                {/* Show inputs whenever that type is selected */}
                ...
                <Form.Group className="mb-3 anim-slide-r">
                  <Form.Label>
                    Photos (at least 2) *
                    <HoldTip text={`Upload clear JPG/PNG images (${MIN_IMAGE_KB}KB–${MAX_LOCAL_PHOTO_MB}MB each). Include a wide shot and a close-up; GPS-tag helps.`} />
                  </Form.Label>
                  <Form.Control
                    type="file"
                    name="photos"
            ...
            {projectType === "Organization" && (
              <>
                ...
                <Form.Group className="mb-3 anim-slide-l">
                  ...
                  <FileHints text={`JPG/PNG, ${MIN_IMAGE_KB}KB–${MAX_ORG_PHOTO_MB}MB each. Provide onset + monitoring images.`} />
                </Form.Group>
...
Useful SQL queries
-- Dashboard list
SELECT id, 'local' AS type, project_title, created_at, status
FROM projects_local
UNION ALL
SELECT id, 'org' AS type, project_title, created_at, status
FROM projects_org
ORDER BY created_at DESC
LIMIT 100;

-- Local detail with files
SELECT p.*, f.kind, f.file_name, f.url, f.size_bytes, f.mime_type, f.created_at AS file_created_at
FROM projects_local p
LEFT JOIN local_files f ON f.project_id = p.id
WHERE p.id = $1;

-- Org detail with files
SELECT p.*, f.kind, f.file_name, f.url, f.size_bytes, f.mime_type, f.created_at AS file_created_at
FROM projects_org p
LEFT JOIN org_files f ON f.project_id = p.id
WHERE p.id = $1;
