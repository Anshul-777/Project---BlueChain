...
const RegisterForm = () => {
  ...
  const validateLocal = () => {
    ...
    if (!Array.isArray(d.photos) || d.photos.length < 2) {
      errs.photos = `Upload at least 2 photos (JPG/PNG, ${MIN_IMAGE_KB} KB to ${MAX_LOCAL_PHOTO_MB} MB each).`;
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (projectType === "Local") {
        ...
        // Optional: SHA-256 (commented for later blockchain anchoring)
        /*
        const core = {
          type: "local",
      ...
      const res = await fetch(`${API_BASE}/api/projects`, { method: "POST", body: payload });
      if (!res.ok) {
  ...
  return (
    <div className={`container py-4 register-form-root ${theme === "dark" ? "theme-dark" : "theme-light"}`} data-theme={theme}>
      <style>{`
        ...
        .checkmark { width: 64px; height: 64px; border-radius: 50%; background: var(--emerald-500); display: inline-flex; align-items: center; justify-content: center; color: white; font-size: 38px; margin-bottom: 12px; box-shadow: 0 8px 20px rgba(16,185,129,0.4); animation: pulse 0.9s ease 1; }
        ...
        .hold-tip-bubble { position: absolute; top: -8px; left: 20px; min-width: 220px; max-width: 320px; background: var(--primary-500); color: white; padding: 8px 10px; border-radius: 8px; font-size: 12px; box-shadow: 0 6px 14px rgba(14,165,233,0.35); animation: tipFade .2s ease both; z-index: 10; }
